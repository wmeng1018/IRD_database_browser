...
$test = $mysqli->query("select id, gender, age, ethnic, phenotype, medicalInfo, familyInfo  from probands where id='$ID' ");
$test1 = $mysqli->query("select v.id, v.geneID, v.gnomad_AF, v.annotation, pv.genotype, pv.seqType, pv.comment from probands as p inner join proband_variant as pv on p.id=pv.probandID inner join variants as v on v.id = pv.variantID where p.id='$ID' ");
...
echo                        "<dt>Race</dt>";
echo                        "<dd> ".$ethnic."</dd>";
echo                        "<dt>Phenotype</dt>";
echo                        "<dd> ".$phenotype."</dd>";
echo                        "<dt>Medical Information</dt>";
echo                        "<dd> ".$medicalInfo."</dd>";
echo                        "<dt>Family Information</dt>";
echo                        "<dd> ".$familyInfo."</dd>";
echo                        "<dt>Age of diagnosis</dt>";
echo                        "<dd> ".$age."</dd>";
echo <<<LABEL
        ...
        <h3>Candidate variants and genotype for proband 
LABEL;
echo "$ID";
echo <<<LABEL
        </h3>
									...
									<div class="col-xs-12">
										...
										<div>
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												<thead>
													<tr>
														<th class="hidden-xs tooltip-table-header" data-tooltip="Variant ID">Variant ID</th>
														<th class="hidden-xs tooltip-table-header" data-tooltip="Gene">Gene<span class="caret"></span></th>
														<th class="hidden-xs tooltip-table-header" data-tooltip="Allele frequency in GnomAD">GnomAD AF<span class="caret"></span></th>
														<th class="hidden-xs tooltip-table-header" data-tooltip="">Annotation<span class="caret"></span></th>
														<th class="hidden-xs tooltip-table-header" data-tooltip="">Genotype<span class="caret"></span></th>
...
LABEL;
					while ($testing1=$test1->fetch_assoc()) {
                                            $id = $testing1["id"];
                                            $geneID = $testing1["geneID"];
					    $gnomad_AF = $testing1["gnomad_AF"];
					    $annotation = $testing1["annotation"];
					    $genotype = $testing1["genotype"];
					    $comment = $testing1["comment"];


      			        echo "<tr>";
                                echo "<td class='hidden-xs'>".'<a href="variant.php?query='.$id.'">'.$id."</td>";
                                echo "<td class='hidden-xs'>".$geneID."</td>";
				echo "<td class='hidden-xs'>".$gnomad_AF."</td>";
	                        echo "<td class='hidden-xs'>".$annotation."</td>";
