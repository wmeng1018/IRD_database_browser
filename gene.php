...
$test1 = $mysqli->query("select DISTINCT id,ens,region,related_disorders,transcript,pli,pdbID,UniProt from genes where id='$query' or ens='$query' limit 1");
$testing1=$test1->fetch_assoc();
$gene = $testing1["id"];
$reg = $testing1["region"];
$ens = $testing1["ens"];
$pdbID = $testing1["pdbID"];
$UniProt = $testing1["UniProt"];
$disease = $testing1["related_disorders"];
$pli = $testing1["pli"];
...
$test2 = $mysqli->query("select id,ens,region,related_disorders,transcript,pli,pdbID,UniProt from genes where id='$gene' ");


#$test3 = $mysqli->query("select v.id,v.geneID,v.annotation,v.cDNA_change,v.aa_change,v.gnomad_AF from variants as v inner join genes as g on v.geneID=g.id where g.id='$gene' ");

$test3 = $mysqli->query("select v.id,v.geneID,v.annotation,v.cDNA_change,v.aa_change,v.gnomad_AF,count(distinct pv.ProbandID) AS NP from variants as v inner join proband_variant as pv on v.id=pv.variantID inner join genes as g on v.geneID=g.id where g.id='$gene' group by pv.variantID");

$test4 = $mysqli->query("select transcript from genes where id='$gene' ");

$test5 = $mysqli->query("select count(v.id) from variants as v inner join genes as g on v.geneID=g.id where g.id='$gene' ");
$testing5=$test5->fetch_assoc();
$VN = $testing5["count(v.id)"];
...
echo                        '<a href="http://genome.ucsc.edu/cgi-bin/hgTracks?hgsid=1205636921_iXfGyhSy6RHJiSFLnA1wOap7BKca&db=hg19&position='.$ens.'" target="_blank">'.$reg;
...
echo                    "<dt>OMIM</dt>";
echo                    '<dd><a href="https://omim.org/search?index=entry&start=1&limit=10&sort=score+desc%2C+prefix_sort+desc&search='.$gene.'" target="_blank">'.$gene.' <i class="fa fa-external-link"></i></a>';
echo                    "</dd>";
echo <<<LABEL
                    <dt>pLI score in GnomAD</dt>
                    <dd>
LABEL;
echo"$pli";
echo <<<LABEL
...
LABEL;
echo            '<p><a href="https://alphafold.com/entry/'.$UniProt.'">AlphaFold prediction</a> (The structure is coloured based on the prediction score)</p>';
echo '<iframe src="./alphafold.php?query='.$UniProt.'"';
echo <<<LABEL
        title="alphafold"
        style="border:none;"
        width="800"
     ...
     reference: {
	   ...
	   "order": 1000000,
           "infoURL": "https://www.ncbi.nlm.nih.gov/gene/?term=$$"
            ...
            tracks: [
                {
                    url: "./vcf/chenlab.vcf.gz",
                    indexURL: "./vcf/chenlab.vcf.gz.tbi",
		    name: "Chen Lab",
			...
			"*": "grey"
                ...
                {
                    url: "./vcf/clinvar_20231007.vcf.gz",
                    indexURL: "./vcf/clinvar_20231007.vcf.gz.tbi",
		    name: "ClinVar 20231007",
                    displayMode: "COLLAPSED",
                    colorBy: "CLNSIG",
                    colorTable: {
                        "Likely_pathogenic": "#ED7D31",
			"Pathogenic": "#ED7D31",
                        "Pathogenic/likely_pathogenic": "#ED7D31",
                        "Pathogenic/Likely_pathogenic": "#ED7D31",			
                        "Likely_benign": "#5B9BD5",
			"Benign": "#5B9BD5",
			"Benign/likely_benign": "#5B9BD5",
                        "Benign/Likely_benign": "#5B9BD5",
                        "*": "grey"
                    },
                    visibilityWindow: -1
									...
									<div class="col-xs-12">
										...
										<div>
											<table id="dynamic-table" class="table table-striped table-bordered table-hover table-sm">
												<thead>
													<tr>
														<th data-tooltip="chromosome:position:reference:alternate (hg19)">Variant ID<span class="caret"></span></th>
														<th data-tooltip="Variant Effect Predictor (VEP) &#xa; annotation using RefSeq.&#xa; Worst across all transcripts of this gene.">VEP Annotation<span class="caret"></span></th>
														<th data-tooltip="HGVS annotation (cDNA change &#xa; otherwise blank)">cDNA&#xa;Change<span class="caret"></span></th>
                                                                                                                <th data-tooltip="HGVS annotation (protein change &#xa;  otherwise blank)">Protein&#xa;Change<span class="caret"></span></th>
														<th data-tooltip="alternate allele frequency in GnomAD">GnomAD AF<span class="caret"></span></th>
                                                                                                                <th data-tooltip="Number of probands &#xa; carry the alternate allele &#xa; in this database">Proband Number<span class="caret"></span></th>
													</tr>
												</thead>
...
LABEL;
					while ($testing3=$test3->fetch_assoc()) {
                                            $ID = $testing3["id"];
                                            $anno = $testing3["annotation"];
					    $cDNA = $testing3["cDNA_change"];
					    $AA = $testing3["aa_change"];
					    $gnomad_AF = $testing3["gnomad_AF"];
					    $NP = $testing3["NP"];


      			        echo "<tr>";
                                echo "<td class='hidden-xs'>".'<a href="variant.php?query='.$ID.'">'.$ID."</a></td>";
				echo "<td class='hidden-xs'>".$anno."</td>";
				echo "<td class='hidden-xs'>".$cDNA."</td>";
				echo "<td class='hidden-xs'>".$AA."</td>";
			        echo "<td class='hidden-xs'>".$gnomad_AF."</td>";
				echo "<td class='hidden-xs'>".$NP."</td>";
      		        	echo "</tr>";
      		         }
